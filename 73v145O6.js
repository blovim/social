...
function youtube_video_list(f, g, h, j, k, l) {
    $.ajax({
        ...
        $.each(c.items, function(i, a) {
            ...
            getwaktu(b, i, g);
            d += '<div class="row valign-wrapper popular_item" data-vvv="' + b + '"><div class="col s4 p_img"><img src="' + c.items[i].snippet.thumbnails.default.url + '" class="responsive-img" alt="ibacor"></div>';
            d += '<div class="col s8 p_content"><a href="#!" class="disabled">'+ c.items[i].snippet.title + '</a><span class="black_text' + i + '"></span></div></div>'
        });
        d += '</div>';
